...
function SlotClientList({
  ...
  const cancel = async (b: Booking) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        alert(e.error ?? t("Couldn't remove the client. Please try again."))
        load() // Revert optimistic update
      }
    } catch {
      alert(t("Network error - couldn't remove the client. Please try again."))
      load()
    } finally {
      onChanged()
    }
  ...
  const move = async (b: Booking, targetSlotId: string) => {
    ...
    if (!res.ok) {
      const e = await res.json().catch(() => ({}))
      alert(e.error ?? t("Couldn't move the booking."))
      load()
    }
    onChanged()
  }
  ...
  const handleAdd = (c: NewClient) => {
    ...
    fetch("/api/admin/bookings", {
      ...
      if (!res.ok) {
        const e = await res.json().catch(() => ({}))
        alert(e.error ?? t("Couldn't add the client."))
      }
  ...
  if (list === null) return <div className="text-[11px] text-gray-400 mt-2">{t("Loading clients…")}</div>

  const targetOptions = targets.map((s) => ({
    id: s.id,
  ...
  return (
    <div className="mt-2 border-t border-gray-200 pt-2 space-y-1.5">
      <div className="flex items-center justify-between">
        <div className="text-[10px] uppercase tracking-wider text-gray-400">{t("Clients")} ({list.length}/{slot.maxCapacity})</div>
...
export default function SchedulePage() {
  ...
  const headerLabel = useMemo(() => {
    if (view === "month") return format(anchor, "MMMM yyyy", { locale: dateLocale })
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (typeof document !== "undefined") {
      ...
      if (openForms.length > 0) {
        ...
        if (notReady) {
          setFormError(t("Finish or cancel the open Add-client form before saving."))
    ...
    const runBatch = Promise.allSettled(
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (r.status === "rejected") {
          failed.push(t("Network error - changes not saved"))
          continue
        }
        if (!r.value.ok) {
          const txt = await r.value.text().catch(() => "")
          let msg = t("Error {status}", { status: r.value.status })
      ...
      {syncError && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[100] max-w-sm w-[calc(100vw-2rem)] px-4 py-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs shadow-lg flex flex-col gap-1 animate-in fade-in slide-in-from-bottom-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold">{t("Save failed - changes reverted")}</span>
          </div>
      ...
      <div className="mb-4 space-y-3">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-xl lg:text-2xl font-bold text-gray-900">{t("Schedule")}</h1>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              title={t("Copy this week's schedule to next week")}
            >
              <Copy size={15} /> <span className="hidden sm:inline">{t("Copy week")}</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex items-stretch gap-2">
          <button
            onClick={handlePrev}
            aria-label={view === "month" ? t("Previous month") : t("Previous week")}
          ...
          <button
            onClick={handleNext}
            aria-label={view === "month" ? t("Next month") : t("Next week")}
            className="flex-1 lg:flex-initial flex items-center justify-center gap-1.5 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 active:scale-[0.98] transition-all"
          >
            ...
            >
                ...
                <div className="mb-4 flex items-center justify-between gap-2 relative">
                  <div>
                    ...
                    <div className="text-sm text-gray-500 mt-0.5">
                      {format(day, dateLocale ? "d MMMM" : "MMMM d", { locale: dateLocale })}
                    </div>
                  </div>
                  {isToday && !blocked && (
                    <span className="text-[10px] font-bold uppercase tracking-wider bg-brand text-white px-2 py-1 rounded-full">{t("Today")}</span>
                  )}
                </div>
              ) : (
              <div className="text-center relative mb-2.5">
                ...
                <button
                  ...
                  title={blocked ? `${t("Blocked")}${blocked.reason ? ": " + blocked.reason : ""}` : t("Block day")}
                  className={cn(
                ...
                <div className="space-y-1.5">
                  {daySlots.map((slot) => {
                    ...
                    return (
                      <div
                        ...
                        title={isHidden ? t("Hidden from clients") : undefined}
                      >
                  ...
                  {daySlots.length < 7 && (
                    <div className="flex items-center justify-center gap-1 py-1.5 text-gray-300 opacity-0 group-hover:opacity-100">
                      <Plus size={11} /><span className="text-[10px]">{t("add")}</span>
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-800">
                {blockModal.existing ? t("Unblock Day") : t("Block Day")}
              </h2>
              <button onClick={() => setBlockModal(null)} className="p-2 hover:bg-gray-100 rounded-lg"><X size={18} /></button>
            </div>
            <p className="text-sm text-gray-500 mb-4">
              {format(new Date(blockModal.date + "T00:00:00"), dateLocale ? "EEEE, d MMMM yyyy" : "EEEE, MMMM d, yyyy", { locale: dateLocale })}
            </p>
            {blockModal.existing ? (
              <p className="text-sm text-gray-600 mb-4">
                {t("This day is currently blocked{reason}. Remove the block?", { reason: blockModal.existing.reason ? ` (${blockModal.existing.reason})` : "" })}
              </p>
            ) : (
              <div className="mb-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">{t("Reason (optional)")}</label>
                <input
                  type="text"
                  value={blockReason}
                  onChange={(e) => setBlockReason(e.target.value)}
                  placeholder={t("e.g. Holiday, Renovation, Private event")}
            ...
            <div className="flex gap-3">
              <button onClick={() => setBlockModal(null)} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50">{t("Cancel")}</button>
              <button
                onClick={handleBlock}
              ...
              >
                {blockModal.existing ? t("Unblock") : t("Block Day")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 touch-none" onTouchMove={(e) => { if (e.target === e.currentTarget) e.preventDefault() }}>
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] flex flex-col overflow-hidden">
            <div className="px-6 pt-5 pb-4 flex items-center justify-between flex-shrink-0 border-b border-gray-100">
              <div>
                <h2 className="text-lg font-semibold text-gray-800">{slots.some((s) => s.date === form.date) ? t("Manage day's sessions") : t("Add Session")}</h2>
                <p className="text-sm text-gray-400 mt-0.5">{format(new Date(form.date + "T00:00:00"), dateLocale ? "EEEE, d MMMM" : "EEEE, MMMM d", { locale: dateLocale })}</p>
              </div>
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  title={blockedDays.some((b) => b.date === form.date) ? t("This day is blocked - tap to unblock") : t("Block this day (no classes can be scheduled)")}
                  className={cn(
                ...
                >
                  {blockedDays.some((b) => b.date === form.date) ? `🔒 ${t("Blocked")}` : t("Block day")}
                </button>
            ...
            >
              <div className="px-6 py-4 space-y-4 overflow-y-auto overflow-x-hidden flex-1 overscroll-contain">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {TIME_PRESETS.map((tp) => {
                      const selected = form.startTimes.includes(tp)
                      const conflictWith = !selected ? overlapsAny(tp, form.startTimes) : null
                        ...
                        <button key={tp} type="button"
                          ...
                          onClick={() => {
                            if (selected) {
                              ...
                              delete newA[tp]
                              setForm({ ...form, startTimes: form.startTimes.filter((x) => x !== tp), assignments: newA })
                              setFormError("")
                            } else {
                              const newTimes = sortTimes([...form.startTimes, tp])
                              const newA = { ...form.assignments }
                              if (!newA[tp]) newA[tp] = { trainerId: "", assistantId: "", classType: "GROUP", publicVisible: true, maxCapacity: 6, repeatWeekly: false }
                    ...
                    <button type="button"
                      onClick={() => {
                        ...
                        if (form.startTimes.includes(form.customTime)) {
                          setFormError(t("{time} is already in the list", { time: formatTime(form.customTime) }))
                          return
                        }
                        const conflict = overlapsAny(form.customTime, form.startTimes)
                        if (conflict) {
                          setFormError(t("{time} overlaps with {range} (classes are 2h long)", { time: formatTime(form.customTime), range: `${formatTime(conflict)}-${formatTime(computeEndTime(conflict))}` }))
                          return
                        }
                        const newA = { ...form.assignments }
                    ...
                    return (form.startTimes.length > 0 || removed.length > 0) && (
                      <div className="mt-3 space-y-2">
                        {form.startTimes.length > 0 && (
                          <div>
                            <div className="text-xs text-gray-500 mb-1.5">{t("Sessions on this day ({count}):", { count: form.startTimes.length })}</div>
                            <div className="space-y-1.5">
                              {form.startTimes.map((st) => {
                                const isExisting = existingTimes.has(st)
                                const assignment = form.assignments[st] ?? { trainerId: "", assistantId: "", classType: "GROUP" as ClassType, publicVisible: true, maxCapacity: 6, repeatWeekly: false }
                                const updateAssignment = (patch: Partial<SlotAssignment>) => {
                                  setForm((prev) => ({
                                    ...prev,
                                    assignments: { ...prev.assignments, [st]: { ...assignment, ...patch } },
                                  }))
                                }
                                const isPrivate = assignment.classType === "PRIVATE"
                                const existingSlot = isExisting ? slots.find((s) => s.date === form.date && s.startTime === st) : null
                                  ...
                                  <div key={st} className={cn(
                                    ...
                                    <div className="flex items-center gap-2 flex-wrap">
                                      <span className={cn(
                                        ...
                                        {formatTime(st)} - {formatTime(computeEndTime(st))}
                                      </span>
                                        ...
                                        {CLASS_TYPES.map((c) => (
                                          <button key={c.value} type="button"
                                            onClick={() => {
                                              if (c.value === "PRIVATE") {
                                                if (bookingCount >= 2) {
                                                  const ok = confirm(t("This session has {count} bookings. Private allows only 1 person - extra clients will be over capacity. Continue?", { count: bookingCount }))
                                                  if (!ok) return
                                                }
                                      ...
                                      <>
                                        ...
                                        <button type="button"
                                          ...
                                          title={assignment.publicVisible ? t("Visible to clients - tap to hide") : t("Hidden from clients - tap to show")}
                                          className={cn(
                                      ...
                                      {isExisting && hasBookings && existingSlot && (
                                        <button type="button"
                                          ...
                                          title={t("Cancel or move this class - every booked client gets a WhatsApp message")}
                                      ...
                                      <button type="button"
                                        ...
                                        onClick={() => {
                                          ...
                                          delete newAssignments[st]
                                          setForm({ ...form, startTimes: form.startTimes.filter((x) => x !== st), assignments: newAssignments })
                                        }}
                                        title={hasBookings
                                          ? t("Has {count} booking(s) - cancel them first or hide the session instead", { count: bookingCount })
                                    ...
                                    <div className="flex items-center gap-2">
                                      <select
                                        ...
                                        title={hasClients ? t("A trainer is required while clients are booked - you can switch trainers but not remove") : t("Trainer")}
                                      ...
                                      >
                                        {!hasClients && <option value="">{t("Unassigned")}</option>}
                                      ...
                                      {assignment.trainerId && form.startTimes.length > 1 &&
                                        ...
                                        <button type="button"
                                          ...
                                          title={t("Apply this trainer to all sessions on this day")}
                                      ...
                                      {hasBookings && (
                                        <span
                                          ...
                                          title={t("{count} confirmed booking(s) - session can't be removed", { count: bookingCount })}
                                        >
                                          🔒 {bookingCount}
                                        </span>
                                      )}
                                    ...
                                    {(() => {
                                      const weekday = format(new Date(form.date + "T00:00:00"), "EEEE", { locale: dateLocale })
                                              ...
                                              {showSeriesWarning
                                                ...
                                                : isPartOfSeries
                                                  ...
                                                  : isExisting
                                                    ...
                                                    : t("Also schedule the next {count} {weekday}s", { count: 12, weekday })}
                                            </div>
                                          </div>
                                        </label>
                                      )
                        ...
                        {removed.length > 0 && (
                          <div>
                            <div className="text-xs text-rose-600 mb-1.5">{t("Will be deleted ({count}):", { count: removed.length })}</div>
              ...
              <div className="px-6 py-4 flex gap-3 flex-shrink-0 border-t border-gray-100">
                <button type="button" onClick={closeModal} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50">{t("Cancel")}</button>
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 touch-none" onTouchMove={(e) => { if (e.target === e.currentTarget) e.preventDefault() }}>
            <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl max-h-[90vh] flex flex-col">
              <div className="px-6 pt-5 pb-4 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
                <div>
                  <h2 className="text-lg font-semibold text-gray-800">{t("Copy week forward")}</h2>
                  <p className="text-xs text-gray-500 mt-0.5">
                    {t("From {from} - {to}", {
                      from: format(copyModal.sourceStart, dateLocale ? "d MMM" : "MMM d", { locale: dateLocale }),
                      to: format(addDays(copyModal.sourceStart, 6), dateLocale ? "d MMM" : "MMM d", { locale: dateLocale }),
                    })}
                    {lastTarget && ` ${t("→ through {date}", { date: format(addDays(lastTarget, 6), dateLocale ? "d MMM" : "MMM d", { locale: dateLocale }) })}`}
                  </p>
                </div>
              ...
              <div className="px-6 py-5 overflow-y-auto">
                {/* Week count selector */}
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t("Copy to how many weeks ahead?")}</label>
                  <div className="flex gap-1.5">
                    {[1, 2, 3, 4, 5].map((n) => (
                      ...
                      >
                        {n === 1 ? t("1 week") : t("{count} weeks", { count: n })}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                {copyModal.loading ? (
                  <div className="text-center py-6 text-sm text-gray-500">{t("Analyzing weeks…")}</div>
                ) : sourceLen === 0 ? (
                  <div className="text-center py-6">
                    <p className="text-sm font-medium text-gray-800">{t("This week has no sessions to copy")}</p>
                  </div>
                ) : (
                  <>
                    {/* Summary */}
                    <div className="mb-3 p-3 rounded-xl bg-brand/5 border border-brand/10">
                      <div className="flex items-baseline gap-2">
                        ...
                        <span className="text-sm text-gray-700">
                          {t("session(s) will be created across {weeks}", {
                            weeks: copyModal.weeksAhead === 1 ? t("1 week") : t("{count} weeks", { count: copyModal.weeksAhead }),
                          })}
                        </span>
                      </div>
                      {totalSkipped > 0 && (
                        <div className="text-xs text-amber-700 mt-1">
                          {t("{count} skipped - already exist at the same date and time", { count: totalSkipped })}
                        </div>
                      )}
                    </div>
                        ...
                        <div key={plan.weekIndex} className="rounded-xl border border-gray-100 overflow-hidden">
                          <div className="px-3 py-2 bg-gray-50 flex items-center justify-between">
                            <div className="text-xs font-semibold text-gray-700">
                              {t("Week {n}", { n: plan.weekIndex })} · {format(plan.targetStart, dateLocale ? "d MMM" : "MMM d", { locale: dateLocale })} - {format(addDays(plan.targetStart, 6), dateLocale ? "d MMM" : "MMM d", { locale: dateLocale })}
                            </div>
                            <div className="text-xs text-gray-500">
                              <span className="text-brand font-medium">+{plan.toCopy.length}</span>
                              {plan.skipped > 0 && <span className="ml-2 text-amber-600">−{plan.skipped} {t("skipped")}</span>}
                            </div>
                          </div>
                          {plan.toCopy.length === 0 ? (
                            <div className="px-3 py-2 text-xs text-gray-400 italic">{t("All sessions already exist this week")}</div>
                          ) : (
                            <div className="divide-y divide-gray-50 max-h-32 overflow-y-auto">
                              {plan.toCopy.map((s) => {
                                const newDate = format(addDays(parseISO(s.date), 7 * plan.weekIndex), dateLocale ? "EEE, d MMM" : "EEE, MMM d", { locale: dateLocale })
                                return (
              ...
              <div className="px-6 pb-5 flex gap-2 flex-shrink-0 border-t border-gray-100 pt-4">
                ...
                >
                  {copyModal.running ? t("Copying…") : totalToCopy > 0 ? t("Copy {count} sessions", { count: totalToCopy }) : t("Nothing to copy")}
                </button>
              </div>
            </div>
          </div>
