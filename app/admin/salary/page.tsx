  ...
  return (
    <div>
      ...
      <div className="mb-6 space-y-3">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold text-gray-900">{t("Salary & Expenses")}</h1>
          <p className="text-gray-500 text-xs lg:text-sm mt-1">{format(monthDate, "LLLL yyyy", { locale: dateLocale })}</p>
        </div>
        {/* Month navigation */}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 lg:gap-4 mb-8">
        <SummaryCard icon={TrendingUp} label={t("Revenue")} value={formatIDR(totalRevenue)} sub={t("From paid bookings")} color="green" />
        <SummaryCard icon={Wallet} label={t("Salaries accrued")} value={formatIDR(totalAccrued)} sub={t("{n} trainer(s)", { n: trainers.length })} color="blue" />
        <SummaryCard icon={CreditCard} label={t("Salaries paid")} value={formatIDR(totalPaid)} sub={t("Balance: {amount}", { amount: formatIDR(totalAccrued - totalPaid) })} color="amber" />
        <SummaryCard icon={Receipt} label={t("Expenses")} value={formatIDR(totalExpenses)} sub={t("{n} item(s)", { n: expenses.length })} color="red" />
        ...
        <SummaryCard
          ...
          sub={t("Accrual: rev - accrued salaries - exp")}
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3 px-1">
          <h2 className="font-semibold text-gray-800 text-base lg:text-lg">{t("Trainer Salaries")}</h2>
          <span className="text-xs text-gray-400">{format(monthDate, "LLLL yyyy", { locale: dateLocale })}</span>
        </div>

        {loading ? (
          <div className="bg-white rounded-2xl shadow-sm"><PetalSpinner /></div>
        ) : trainers.length === 0 ? (
          <div className="bg-white rounded-2xl py-12 text-center text-gray-400 text-sm shadow-sm">{t("No trainers found.")}</div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 lg:gap-4">
            {trainers.map((tr) => (
              <div key={tr.id} className="bg-white rounded-2xl p-4 lg:p-5 shadow-sm">
                {/* Trainer header */}
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="min-w-0">
                    <div className="font-semibold text-gray-900 truncate">{tr.name}</div>
                    <div className="text-xs text-gray-400 mt-0.5">{t("{rate}% commission · {n} sessions", { rate: tr.commissionRate, n: tr.sessions })}</div>
                  </div>
                  <button
                    onClick={() => { setPayModal(tr); setPayAmount(String(tr.balance > 0 ? tr.balance : "")); setPayFromSafe(false); setPayMethod("CASH") }}
                    disabled={tr.balance <= 0}
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  <Stat label={t("Revenue")} value={formatIDR(tr.revenue)} />
                  <Stat label={t("Accrued")} value={formatIDR(tr.accrued)} accent="brand" hint={t("+{amount} comm.", { amount: formatIDR(tr.commission) })} />
                  <Stat label={t("Paid out")} value={formatIDR(tr.paid)} />
                  <Stat
                    label={t("Balance")}
                    value={tr.balance === 0 ? t("Settled") : formatIDR(tr.balance)}
                    accent={tr.balance > 0 ? "amber" : tr.balance < 0 ? "red" : "muted"}
                  />
                </div>
                ...
                {tr.payments.length > 0 && (
                  <div className="pt-3 border-t border-gray-100">
                    <div className="text-[10px] uppercase tracking-wide text-gray-400 mb-2">{t("Payments this month")}</div>
                    <div className="flex flex-wrap gap-1.5">
                      {tr.payments.map((p) => (
                        <div key={p.id} className="flex items-center gap-1.5 bg-gray-50 border border-gray-100 rounded-lg px-2.5 py-1.5 text-xs">
                          {p.kind === "accrual" ? (
                            <span className="text-amber-600 font-medium">{t("+{amount} accrued", { amount: formatIDR(p.amount) })}</span>
                          ) : (
                            <span className="text-brand font-medium">{t("{amount} paid", { amount: formatIDR(p.amount) })}</span>
                          )}
                          <span className="text-gray-300">·</span>
                          <span className="text-gray-400">{format(new Date(p.createdAt), "d MMM", { locale: dateLocale })}</span>
                  ...
                  <div className="pt-3 mt-3 border-t border-gray-100">
                    <button
                      ...
                      onClick={() => setExpandedIds((prev) => {
                        ...
                        n.has(tr.id) ? n.delete(tr.id) : n.add(tr.id)
                        return n
                      })}
                      className="w-full flex items-center justify-between text-[11px] uppercase tracking-wide text-gray-400 hover:text-gray-600"
                    >
                      <span>{t("Commission breakdown · {n} classes", { n: tr.breakdown.length })}</span>
                      <ChevronDown size={14} className={cn("transition-transform", expandedIds.has(tr.id) && "rotate-180")} />
                    </button>
                    {expandedIds.has(tr.id) && (
                          ...
                          <div key={r.bookingId} className="flex items-center gap-2 px-3 py-2 border-b border-gray-50 last:border-b-0 text-xs">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-[10px] text-gray-400 mt-0.5">
                                {format(new Date(r.date), dateLocale ? "d MMM" : "MMM d", { locale: dateLocale })} · {r.startTime} · {classTypeLabel(r.classType)}
                                {r.role === "assistant" && ` · ${t("assisted")}`}
                              </div>
                            </div>
                            <div className="text-right flex-shrink-0">
                              <div className="font-semibold text-gray-900 whitespace-nowrap">+{formatIDR(r.commission)}</div>
                              <div className="text-[10px] text-gray-400 whitespace-nowrap">{t("{rate}% of {amount}", { rate: r.rate, amount: formatIDR(r.amount) })}{r.tier && r.tier !== "FULL" ? ` (${PRICE_TIER_LABEL[r.tier] ?? r.tier})` : ""} · {paymentTypeLabel(r.paymentType)}</div>
                            </div>
                          </div>
                        ))}
                        <div className="flex items-center justify-between px-3 py-2 bg-gray-50 text-xs">
                          <span className="font-semibold text-gray-600">{t("Total commission")}</span>
                          <span className="font-bold text-gray-900">{formatIDR(tr.commission)}</span>
                        </div>
                      </div>
                    )}
                  </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3 px-1 gap-3">
          <div className="min-w-0">
            <h2 className="font-semibold text-gray-800 text-base lg:text-lg">{t("Expenses")}</h2>
            <p className="text-xs text-gray-400 mt-0.5 truncate">{t("Total: {amount}", { amount: formatIDR(totalExpenses) })}</p>
          </div>
          <button
        ...
        {expenses.length === 0 ? (
          <div className="bg-white rounded-2xl py-10 text-center text-gray-400 text-sm shadow-sm">
            {t("No expenses recorded for {month}.", { month: format(monthDate, "LLLL yyyy", { locale: dateLocale }) })}
          </div>
        ) : (
              ...
              <div key={exp.id} className="bg-white rounded-2xl p-4 shadow-sm flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-600 rounded-full">{t(exp.category)}</span>
                    <span className="text-xs text-gray-400">{format(new Date(exp.date + "T00:00:00"), "d MMM yyyy", { locale: dateLocale })}</span>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 sm:p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl p-5 sm:p-6 w-full max-w-sm shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <div className="min-w-0">
                <h2 className="text-lg font-semibold text-gray-800">{t("Pay Out")}</h2>
                <p className="text-sm text-gray-400 mt-0.5 truncate">{payModal.name} · {format(monthDate, "MMM yyyy", { locale: dateLocale })}</p>
              </div>
            ...
            <div className="bg-gray-50 rounded-xl px-4 py-3 mb-4 text-sm space-y-1.5">
              <div className="flex justify-between gap-3"><span className="text-gray-500">{t("Commission ({rate}%)", { rate: payModal.commissionRate })}</span><span className="font-medium">+{formatIDR(payModal.commission)}</span></div>
              <div className="flex justify-between gap-3"><span className="text-gray-500">{t("Already paid")}</span><span className="font-medium text-green-600">−{formatIDR(payModal.paid)}</span></div>
              <div className="flex justify-between gap-3 border-t border-gray-200 pt-1.5 mt-1"><span className="font-medium text-gray-700">{t("Balance due")}</span><span className="font-bold text-brand">{formatIDR(payModal.balance)}</span></div>
            </div>
            ...
            <form onSubmit={handlePay} className="space-y-3">
              ...
              {!payFromSafe && (
                <div>
                  ...
                  >
                    ...
                    <option value="EDC">{t("Card")}</option>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand"
                  placeholder={t("e.g. Cash, transfer #123")}
                />
              </div>
              ...
              {safeBalances !== null && (safeBalances.get(payModal.id) ?? 0) > 0 && (
                <label className="flex items-start gap-2.5 rounded-xl border border-gray-200 px-3.5 py-2.5 cursor-pointer hover:border-brand/40">
                  ...
                  <span className="text-sm text-gray-700">
                    {t("Pay from {name}'s safe", { name: payModal.name.split(/\s+/)[0] })}
                    <span className="block text-xs text-gray-400 mt-0.5">
                      {t("{amount} cash available in the safe", { amount: formatIDR(safeBalances.get(payModal.id) ?? 0) })}
                    </span>
                  </span>
                </label>
              )}
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setPayModal(null)} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50">{t("Cancel")}</button>
                <button type="submit" disabled={paying} className="flex-1 bg-brand text-white py-2.5 rounded-xl text-sm font-medium hover:bg-brand-dark disabled:opacity-60">
                  {paying ? t("Saving...") : t("Confirm Payment")}
                </button>
              </div>
            </form>
          </div>
            ...
            <form onSubmit={handleAddExpense} className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand"
                  placeholder={t("e.g. Monthly studio rent")}
                />
              </div>
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setShowExpenseForm(false)} className="flex-1 border border-gray-200 text-gray-600 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50">{t("Cancel")}</button>
                <button type="submit" disabled={savingExp} className="flex-1 bg-brand text-white py-2.5 rounded-xl text-sm font-medium hover:bg-brand-dark disabled:opacity-60">
                  {savingExp ? t("Saving...") : t("Add Expense")}
                </button>
              </div>
            </form>
          </div>
