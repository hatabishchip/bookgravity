...
export default async function StudioBookingPage({
  ...
  const studio = await prisma.studio.findFirst({
    where: { slug },
    select: { id: true, name: true, slug: true, country: true, city: true, logoUrl: true, coverUrl: true, locationUrl: true, whatsappEnabled: true, currency: true, groupPrice: true },
  })
  if (!studio) notFound()
